...
function sanitizePublicText(value) {
  return String(value)
    ...
    .replace(/\b(buy|bought|sell|sold|add|added|trim|trimmed|exit|exited)\s+\d[\d,]*(?:\.\d+)?\s+(?=(?:at|@)\s*(?:USD\s*)?(?:HK\$|[$¥])?\d)/gi, "$1 position ")
    .replace(/\bremaining\s+\d[\d,]*(?:\.\d+)?\b/gi, "remaining position")
    .replace(/\boriginal\s+\d[\d,]*(?:\.\d+)?\b/gi, "original position")
