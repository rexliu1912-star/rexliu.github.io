...
const DRIVER_FACTOR_DEFS = {
  "ai capex": { label_en: "AI Capex", label_zh: "AI capex", type: "growth", evidence_en: "AI build-out remains the dominant shared demand driver.", evidence_zh: "AI 建设仍是组合内最核心的共同需求因子。" },
  "grid capex": { label_en: "Grid Capex", label_zh: "电网资本开支", type: "growth", evidence_en: "Grid spending and power bottlenecks support equipment demand.", evidence_zh: "电网投资与电力瓶颈支撑设备需求。" },
  "export orders": { label_en: "Export Orders", label_zh: "海外订单", type: "growth", evidence_en: "Export order momentum is tracked through position news and scan scores.", evidence_zh: "海外订单动能通过持仓新闻与扫描评分跟踪。" },
  "free cash flow": { label_en: "Free Cash Flow", label_zh: "自由现金流", type: "quality", evidence_en: "Quality and cash generation matter more in defensive regimes.", evidence_zh: "防御市场里，质量与现金流权重更高。" },
  "rates": { label_en: "Rates", label_zh: "利率", type: "macro", signal: "tnx", evidence_en: "10Y Treasury signal is the live proxy for duration pressure.", evidence_zh: "10 年美债信号是久期压力的实时代理。" },
  "regulation": { label_en: "Regulation", label_zh: "监管", type: "policy", signal: "policy", evidence_en: "Policy/regulatory status is read from the macro thermometer.", evidence_zh: "政策/监管状态来自宏观温度计。" },
  "approvals": { label_en: "Approvals", label_zh: "核准节奏", type: "policy", signal: "policy", evidence_en: "Approval cadence is policy-sensitive.", evidence_zh: "核准节奏对政策环境敏感。" },
  "ppa demand": { label_en: "PPA Demand", label_zh: "长协/PPA需求", type: "growth", evidence_en: "Large-load power buyers keep PPA demand on the radar.", evidence_zh: "大负荷用电方让长协/PPA需求保持重要。" },
  "nuclear policy": { label_en: "Nuclear Policy", label_zh: "核电政策", type: "policy", signal: "policy", evidence_en: "Nuclear exposure is primarily driven by policy support and permitting.", evidence_zh: "核电敞口主要受政策支持与审批驱动。" },
  "usd": { label_en: "USD", label_zh: "美元", type: "macro", signal: "dxy", evidence_en: "DXY signal captures currency headwind/tailwind.", evidence_zh: "DXY 信号捕捉美元带来的顺风/逆风。" },
  "ev demand": { label_en: "EV Demand", label_zh: "电动车需求", type: "growth", evidence_en: "EV demand is tracked through Tesla news and momentum.", evidence_zh: "电动车需求通过特斯拉新闻与动量跟踪。" },
  "ai/robotics": { label_en: "AI/Robotics", label_zh: "AI/机器人", type: "growth", evidence_en: "AI/robotics optionality remains the key upside driver.", evidence_zh: "AI/机器人期权仍是主要上行驱动。" },
  "gas spreads": { label_en: "Gas Spreads", label_zh: "天然气价差", type: "commodity", evidence_en: "Gas spread cycle is tracked by position news and commodity context.", evidence_zh: "天然气价差周期通过持仓新闻和商品环境跟踪。" },
  "data center demand": { label_en: "Data Center Demand", label_zh: "数据中心需求", type: "growth", evidence_en: "Data-center demand is the direct read-through from AI capex.", evidence_zh: "数据中心需求是 AI capex 的直接映射。" },
  "supply chain": { label_en: "Supply Chain", label_zh: "供应链", type: "quality", evidence_en: "Supply-chain execution is watched through semiconductor news.", evidence_zh: "供应链执行通过半导体新闻跟踪。" },
  "liquidity": { label_en: "Liquidity", label_zh: "流动性", type: "macro", evidence_en: "Liquidity combines DXY, NAAIM and policy lights.", evidence_zh: "流动性综合 DXY、NAAIM 与政策信号。" },
  "semiconductor cycle": { label_en: "Semiconductor Cycle", label_zh: "半导体周期", type: "cycle", evidence_en: "Chip-cycle health is proxied by semiconductor basket ratings and momentum.", evidence_zh: "半导体周期由半导体篮子评分与动量代理。" },
  "software earnings": { label_en: "Software Earnings", label_zh: "软件盈利", type: "quality", evidence_en: "Software earnings quality is tracked through the tech basket.", evidence_zh: "软件盈利质量通过科技篮子跟踪。" },
  "optical interconnect": { label_en: "Optical Interconnect", label_zh: "光互连周期", type: "cycle", evidence_en: "Optical interconnect is watched as an AI infrastructure sub-cycle.", evidence_zh: "光互连作为 AI 基建子周期跟踪。" },
  "autonomy/robotics": { label_en: "Autonomy/Robotics", label_zh: "自动驾驶/机器人", type: "growth", evidence_en: "Autonomy and robotics adoption drive optionality.", evidence_zh: "自动驾驶与机器人采用驱动期权价值。" },
  "industrial adoption": { label_en: "Industrial Adoption", label_zh: "工业采用", type: "growth", evidence_en: "Industrial adoption is tracked through order/news momentum.", evidence_zh: "工业采用通过订单与新闻动能跟踪。" },
};

function normalizeDriverFactor(factor) {
  return String(factor || "").trim().toLowerCase();
}

function driverLightToStatus(light) {
  if (light === "green") return "tailwind";
  if (light === "red") return "headwind";
  return "neutral";
}

function statusScore(status) {
  return status === "tailwind" ? 1 : status === "headwind" ? -1 : 0;
}
...
function buildDriverSignals({ positions, crypto, regime, quantRatings, newsDigest, previousSignals = null }) {
  const factorMap = new Map();
  const addExposure = (factor, position, zhFactor = null) => {
    const key = normalizeDriverFactor(factor);
    if (!key) return;
    const def = DRIVER_FACTOR_DEFS[key] || { label_en: factor, label_zh: zhFactor || factor, type: "custom" };
    if (!factorMap.has(key)) {
      factorMap.set(key, { key, label_en: def.label_en, label_zh: def.label_zh || zhFactor || def.label_en, type: def.type || "custom", def, exposures: [] });
    }
    factorMap.get(key).exposures.push(position);
  };

  for (const p of positions || []) {
    const en = p.driver_factors_en || [];
    const zh = p.driver_factors_zh || [];
    en.forEach((factor, i) => {
      addExposure(factor, { symbol: p.symbol, name: p.name || p.name_en || p.symbol, market: p.market, kind: "position" }, zh[i]);
    });
  }
  for (const p of crypto?.positions || []) {
    ...
    const zh = p.driver_factors_zh || [];
    en.forEach((factor, i) => {
      addExposure(factor, { symbol: p.symbol, name: p.symbol, market: "crypto", kind: "crypto" }, zh[i]);
    });
  }

  const regimeSignals = regime?.current?.signals || {};
  const ratings = quantRatings?.ratings || {};
  const news = newsDigest?.positions || {};
  const lastByKey = new Map((previousSignals?.factors || []).map((f) => [f.key, f]));

  const factors = [...factorMap.values()].map((factor) => {
    const def = factor.def;
    const impacted = [...new Map(factor.exposures.map((p) => [p.symbol, p])).values()];
    let status = "neutral";
    let confidence = "medium";
    let evidence_en = def.evidence_en || "Tracked from portfolio factor exposures, latest scan data and position news.";
    let evidence_zh = def.evidence_zh || "由持仓因子暴露、最新扫描数据与持仓新闻共同跟踪。";

    if (def.signal && regimeSignals[def.signal]) {
      const sig = regimeSignals[def.signal];
      status = driverLightToStatus(sig.light);
      evidence_en = `${def.label_en}: ${sig.label} is ${sig.value} (${sig.light}).`;
      evidence_zh = `${def.label_zh}: ${sig.label} 当前为 ${sig.value}（${sig.light}）。`;
      confidence = "high";
    } else if (factor.key === "liquidity") {
      const lights = [regimeSignals.dxy?.light, regimeSignals.naaim?.light, regimeSignals.policy?.light].filter(Boolean);
      const score = lights.reduce((s, light) => s + statusScore(driverLightToStatus(light)), 0);
      status = score >= 1 ? "tailwind" : score <= -1 ? "headwind" : "neutral";
      evidence_en = `Liquidity basket: DXY/NAAIM/policy lights = ${lights.join("/") || "n/a"}.`;
      evidence_zh = `流动性篮子：DXY/NAAIM/政策信号 = ${lights.join("/") || "n/a"}。`;
      confidence = lights.length >= 2 ? "high" : "medium";
    } else {
      const composites = impacted.map((p) => ratings[p.symbol]?.composite).filter((n) => Number.isFinite(n));
      const momentum = impacted.map((p) => ratings[p.symbol]?.factors?.momentum?.score).filter((n) => Number.isFinite(n));
      const sentiments = impacted.map((p) => news[p.symbol]?.sentiment).filter(Boolean);
      const sentimentScore = sentiments.reduce((s, v) => s + (v === "positive" ? 1 : v === "negative" ? -1 : 0), 0);
      const q = avg(composites);
      const m = avg(momentum);
      const combined = (q != null ? q - 3 : 0) + (m != null ? (m - 3) * 0.7 : 0) + sentimentScore * 0.35;
      status = combined >= 0.75 ? "tailwind" : combined <= -0.75 ? "headwind" : "neutral";
      const rated = composites.length;
      const sentimentLabel = sentiments.length ? sentiments.join("/") : "no fresh news sentiment";
      evidence_en = `${rated}/${impacted.length} exposures rated; avg quant ${q != null ? q.toFixed(1) : "n/a"}, momentum ${m != null ? m.toFixed(1) : "n/a"}, news ${sentimentLabel}.`;
      evidence_zh = `${rated}/${impacted.length} 个敞口有量化评分；平均评分 ${q != null ? q.toFixed(1) : "n/a"}，动量 ${m != null ? m.toFixed(1) : "n/a"}，新闻 ${sentimentLabel}。`;
      confidence = rated >= Math.max(1, Math.ceil(impacted.length / 2)) || sentiments.length >= 2 ? "medium" : "low";
    }

    const previous = lastByKey.get(factor.key);
    const prevScore = previous ? statusScore(previous.status) : 0;
    const curScore = statusScore(status);
    const change = curScore > prevScore ? "up" : curScore < prevScore ? "down" : "flat";
    return {
      key: factor.key,
      ...
      change,
      confidence,
      impacted: impacted.slice(0, 8),
      impacted_count: impacted.length,
      evidence_en,
      evidence_zh,
    };
  }).sort((a, b) => {
    const statusRank = { headwind: 0, tailwind: 1, neutral: 2 };
    return statusRank[a.status] - statusRank[b.status] || b.impacted_count - a.impacted_count || a.label_en.localeCompare(b.label_en);
  });

  const counts = factors.reduce((acc, f) => {
    acc[f.status] = (acc[f.status] || 0) + 1;
    return acc;
  }, { tailwind: 0, neutral: 0, headwind: 0 });
  const netScore = factors.reduce((s, f) => s + statusScore(f.status) * Math.min(3, f.impacted_count), 0);
  const posture = netScore >= 4 ? "constructive" : netScore <= -4 ? "defensive" : "balanced";
  const postureLabels = {
    constructive: { en: "Constructive", zh: "偏顺风" },
    balanced: { en: "Balanced", zh: "均衡" },
    defensive: { en: "Defensive", zh: "偏逆风" },
  };
  const now = new Date();
  return {
    generated_at: now.toISOString(),
    source: {
      builder: "scripts/build-portfolio-public.mjs",
      inputs: ["portfolio-overrides.driver_factors", "macro thermometer", "quant-ratings", "portfolio-news-digest"],
      news_digest_at: newsDigest?.generated_at || null,
      quant_ratings_date: quantRatings?.date || null,
    },
    summary: {
      posture,
      label_en: postureLabels[posture].en,
      label_zh: postureLabels[posture].zh,
      net_score: netScore,
      tailwinds: counts.tailwind || 0,
      neutral: counts.neutral || 0,
      headwinds: counts.headwind || 0,
      weekly_signal_en: `${counts.tailwind || 0} tailwinds / ${counts.headwind || 0} headwinds across ${factors.length} tracked drivers`,
      weekly_signal_zh: `${factors.length} 个因子中：${counts.tailwind || 0} 个顺风 / ${counts.headwind || 0} 个逆风`,
    },
    factors,
  };
}
...
async function main() {
  ...
  const previousPublic = await readJson(OUTPUT_PATH, null);
  const driverSignals = buildDriverSignals({
    positions: filteredPublicPositions,
    crypto,
    regime,
    quantRatings,
    newsDigest,
    previousSignals: previousPublic?.driver_signals || null,
  });
  console.log(`🧭 Driver Radar: ${driverSignals.factors.length} factors, posture=${driverSignals.summary.posture}, net=${driverSignals.summary.net_score}`);
