...
**当前现状（2026-03-09）：**
- 项目里已经有 `Search.astro`
- 生产构建后会自动跑 `pagefind --site dist`
- 说明底层静态搜索能力已经具备，缺的是入口、信息架构和结果范围控制

**推荐方案：Pagefind（优先）**
- **原因**：零后端、适合 Astro 静态站、现成依赖已安装、支持中英文混合检索、上线成本最低
- **接入方式**：
  1. 先在 `/lab/` 首页和各列表页接入搜索入口
  2. 搜索范围先覆盖：`digest`、`snek-daily`、`builder-log`、`ai-timeline`
  3. 搜索结果按内容类型分组展示（而不是一锅端）
  4. 给 AI Timeline 补充 `company` / `category` / `date` 可读摘要，提升命中质量

**不推荐作为第一步的方案：**
- **Fuse.js 前端 JSON 搜索**：灵活，但内容一多 bundle 会膨胀，还得自己造索引和 UI
- **Algolia/Meilisearch/Typesense**：体验更强，但要引入服务端或第三方托管，现阶段过重
...
- ✅ Builder's Log
- ✅ AI Timeline
- ⏸️ 暂不纳入 Posts / Notes / Tweet Archive

**Phase 1 验收标准：**
- `/lab/` 和 4 个子栏目都能打开统一搜索框
- 搜索 `OpenAI` / `Claude` / `SNEK` / `BTC` / `Master Account` 等关键词时，能命中对应内容
- 搜索结果能看出内容类型、标题、日期和一行摘要
- 不引入任何额外后端服务
...
- [ ] **Lab 搜索接入**（优先 Pagefind，先覆盖 4 个 Lab 栏目）

### Next（接下来一周）
- [ ] SNEK Daily 独立 RSS
- [ ] 搜索结果页信息分组优化（Digest / SNEK / Timeline / Builder's Log）
- [ ] AI Timeline 搜索友好度优化（摘要 / company / category 暴露更完整）
- [ ] Digest 库存继续积累到「看起来像一个活着的栏目」
